...
quill.on('text-change', function(delta, oldDelta, source) {
    ...
    let line = getLine();
    let l = `line-${line}`;
    let ep = quill.getSelection().index;
    ...
    else
    {
        ...
        ar.forEach(item=>{
          if(item.name == name && item.ep==ep-1){ //insert at end same user
            ...
            if(ope2.hasOwnProperty('insert') && ope2.insert=="↵")
                console.log('Enter');
        ...
        if(flag==0){ //new record for same user typing elsewhere in same line or a new user typing
          ...
          if(flag==2){
            ...
            if(j!=-1)
              ar.splice(j,1); //removing the previous item as it has been broken into several parts
          }
          else
